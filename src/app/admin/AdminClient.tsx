...
function ServicesTab({
  ...
  // 以 telegram_id 分組，收集每位客戶的所有預約
  const confirmedAppts = appointments.filter(
    (a) => a.status === "confirmed" && a.telegram_id,
  );
  const userAppointments: Record<string, any[]> = {};
  confirmedAppts.forEach((a) => {
    if (!userAppointments[a.telegram_id]) userAppointments[a.telegram_id] = [];
    userAppointments[a.telegram_id].push(a);
  });
  ...
  return (
    <div>
      ...
      <h3 className="font-bold text-red-600 mb-3">
        🚫 黑名單（{blacklist.length} 人）
      </h3>
      {loading ? (
        <p className="text-gray-400">載入中...</p>
      ) : blacklist.length === 0 ? (
        <p className="text-gray-400 mb-6">目前沒有黑名單客戶</p>
      ) : (
            ...
            >
              <div>
                <p className="font-bold text-red-800">
                  TG ID：
                  <code className="bg-red-100 px-1 rounded text-sm">
                    {u.telegram_id}
                  </code>
                </p>
                ...
                <p className="text-xs text-red-400 mt-0.5">
                  加入時間：{new Date(u.created_at).toLocaleString("zh-TW")}
                </p>
              </div>
              <button
                onClick={() => removeFromBlacklist(u.telegram_id)}
                className="bg-white border border-red-300 text-red-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-100"
              >
                解除封鎖
              </button>
            </div>
      ...
      <h3 className="font-bold text-primary mb-3">
        📋 客戶記錄（{Object.keys(userAppointments).length} 人）
      </h3>
      {Object.keys(userAppointments).length === 0 ? (
        <p className="text-gray-400">目前沒有已預約的客戶</p>
      ) : (
        <div className="space-y-2">
          {Object.entries(userAppointments)
            .filter(([tgId]) => !blacklist.some((b) => b.telegram_id === tgId))
            .map(([tgId, apts]) => {
              const techNames = [
                ...new Set(
                  apts.map((a: any) => {
                    const t = technicians.find(
                      (t: any) => t.id === a.technician_id,
                    );
                    return t?.nickname || "未知技師";
                  }),
                ),
              ].join(", ");
              const dates = apts
                .map(
                  (a: any) =>
                    `${new Date(a.date).toLocaleDateString("zh-TW")} ${a.start_time?.slice(0, 5)}`,
                )
                .join("、");
              return (
                <div
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">📅 {dates}</p>
                  ...
                  <button
                    onClick={() => addToBlacklist(tgId, "管理員封鎖")}
